...
const filteredComments = computed(() => {
    return props.comments.filter((c) => {
        ...
        const matchesSearch = c.content.toLowerCase().includes(query) || c.author_name.toLowerCase().includes(query);
...
watch([() => props.searchQuery, () => props.filterSentiment, () => props.comments.length], () => {
    currentPage.value = 1;
});
...
const goToPage = (page) => {
    ...
    if (Number.isInteger(numeric) && numeric >= 1 && numeric <= totalPages.value) {
...
const getConfidencePercent = (analysis) => {
    const raw = typeof analysis?.confidence === "number" ? analysis.confidence : typeof analysis?.score === "number" ? analysis.score * 100 : 0;
...
<template>
    <div class="flex h-full flex-col overflow-hidden rounded-3xl border border-gray-100 bg-white shadow-sm dark:border-gray-700 dark:bg-gray-800">
        <div class="no-print flex flex-wrap items-center justify-between gap-4 border-b border-gray-200 bg-white p-5 dark:border-gray-700 dark:bg-gray-800">
            <div class="flex gap-3 flex-1 flex-wrap md:flex-nowrap">
                ...
                <div class="flex items-center rounded-full border border-gray-200 bg-gray-100 px-5 py-3 transition focus-within:ring-2 focus-within:ring-blue-500/20 dark:border-gray-700 dark:bg-gray-700 dark:focus-within:ring-blue-400/20">
                    <select :value="filterSentiment" @change="emit('update:filterSentiment', $event.target.value)" class="min-w-[140px] cursor-pointer bg-transparent pr-3 text-sm font-medium text-gray-900 outline-none dark:text-gray-100">
                        <option value="all">
            ...
            <div class="flex flex-wrap items-center justify-end gap-2">
                <button type="button" @click="handleExport" class="flex items-center gap-2 rounded-full bg-green-600 px-5 py-2.5 text-xs font-bold uppercase tracking-wider text-white shadow-sm transition hover:bg-green-700 hover:shadow-md active:translate-y-px">
                    <span>📥</span> {{ t("commentsTable.exportButton") }}
                </button>
                <button type="button" @click="handlePrint" class="flex items-center gap-2 rounded-full bg-slate-700 px-5 py-2.5 text-xs font-bold uppercase tracking-wider text-white shadow-sm transition hover:bg-slate-800 hover:shadow-md active:translate-y-px">
        ...
        <div v-if="filteredComments.length > 0" class="min-h-136">
            <transition name="fade" mode="out-in">
                <div :key="`${currentPage}-${searchQuery}-${filterSentiment}`" class="custom-scrollbar overflow-x-auto">
                    <table class="w-full border-collapse text-left">
                        <thead class="border-b border-gray-200 bg-gray-50 dark:border-gray-700 dark:bg-gray-800">
                            <tr>
                                <th class="border-r border-gray-200 p-5 text-xs font-bold uppercase tracking-widest text-gray-500 dark:border-gray-700 dark:text-gray-400">
                                    {{ t("commentsTable.columns.author") }}
                                </th>
                                <th class="w-1/2 border-r border-gray-200 p-5 text-xs font-bold uppercase tracking-widest text-gray-500 dark:border-gray-700 dark:text-gray-400">
                                    {{ t("commentsTable.columns.comment") }}
                                </th>
                                <th class="p-5 text-center text-xs font-bold uppercase tracking-widest text-gray-500 dark:text-gray-400">
                                    {{ t("commentsTable.columns.aiAnalysis") }}
                                </th>
                            </tr>
                        </thead>
                        <tbody class="divide-y divide-gray-200 dark:divide-gray-700">
                            <tr v-for="c in paginatedComments" :key="c.comment_id" class="group transition-colors odd:bg-white even:bg-gray-50/70 hover:bg-blue-50/70 dark:odd:bg-gray-800 dark:even:bg-gray-800/50 dark:hover:bg-gray-700/60">
                                <td class="border-r border-gray-200 p-5 text-sm font-bold text-gray-900 dark:border-gray-700 dark:text-gray-100">
                                    {{ c.author_name }}
                                </td>

                                <td class="border-r border-gray-200 p-5 text-sm leading-relaxed text-gray-500 dark:border-gray-700 dark:text-gray-400">
                                    <div>
                                        <template v-for="(part, idx) in highlightTextParts(c.content, searchQuery)" :key="`${c.comment_id}-part-${idx}`">
                                            <mark v-if="part.matched" class="rounded bg-yellow-200 px-0.5 font-semibold text-black">
                                                {{ part.value }}
                                            </mark>
                                ...
                                <td class="p-5">
                                    <div class="flex flex-col items-center gap-3">
                                        <span
                                            :class="{
                                                'text-green-700 bg-green-100 border-green-200 dark:text-green-400 dark:bg-green-900/30 dark:border-green-500/50': c.analysis?.sentiment === 'positive',
                                                'text-red-700 bg-red-100 border-red-200 dark:text-red-400 dark:bg-red-900/30 dark:border-red-500/50': c.analysis?.sentiment === 'negative',
                                                'text-gray-700 bg-gray-100 border-gray-200 dark:text-gray-300 dark:bg-gray-800/50 dark:border-gray-500/50': c.analysis?.sentiment === 'neutral',
                                            }"
                                        ...
                                        <div class="relative w-full max-w-[100px] group/bar">
                                            <div class="h-2 w-full overflow-hidden rounded-full bg-gray-200 dark:bg-gray-700/70">
                                                <div
                                            ...
                                            <p class="mt-1 text-center font-mono text-[9px] text-gray-500 dark:text-gray-400">{{ getConfidencePercent(c.analysis) }}%</p>
                                        </div>
        ...
        <div v-if="filteredComments.length > 0" class="no-print border-t border-gray-200 bg-white px-4 py-4 dark:border-gray-700 dark:bg-gray-800">
            <div class="flex w-full items-center justify-center gap-4">
                ...
                <div class="flex items-center gap-2">
                    <template v-for="(item, index) in visiblePageItems" :key="item === '...' ? `ellipsis-${index}` : `page-${item}`">
                        <button
                            @click="goToPage(item)"
                            :disabled="item === '...'"

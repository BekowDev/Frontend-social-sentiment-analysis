...
const handleDeleteAccount = async () => {
    ...
    const userInput = prompt(
        `${t("common.confirmDeleteAccountTypeWord")}\n\n${confirmationWord}`,
        "",
    );
    if (userInput === null) {
        return;
...
<template>
    ...
    >
        ...
        >
            <RouterLink
                :to="{ name: 'Landing' }"
                class="inline-flex cursor-pointer items-center rounded-full px-2 py-1 transition hover:bg-gray-100 dark:hover:bg-gray-800"
                aria-label="MoodFlow home"
            >
                <span
                    class="text-2xl font-black tracking-tight sm:text-[1.7rem]"
                >
                    <span class="text-gray-900 dark:text-white">Mood</span>
                    <span
                        class="bg-linear-to-r from-blue-500 to-indigo-600 bg-clip-text text-transparent"
                        >Flow</span
                    >
                </span>
            </RouterLink>
            ...
            <div class="flex flex-wrap items-center justify-end gap-2 sm:gap-3">
                ...
                <div v-if="auth.isLoggedIn" ref="profileMenuRef" class="relative">
                    <button
                        class="inline-flex h-10 w-10 items-center justify-center rounded-full bg-gray-100 text-gray-700 transition hover:bg-gray-200 dark:bg-gray-800 dark:text-gray-100 dark:hover:bg-gray-700"
                    ...
                    >
                        ...
                        >
                            <path
                                ...
                                d="M15.75 6.75a3.75 3.75 0 1 1-7.5 0 3.75 3.75 0 0 1 7.5 0Zm-10.5 13.5a6.75 6.75 0 0 1 13.5 0"
                            />
                        </svg>
                    </button>
                    ...
                    <div
                        ...
                        class="absolute right-0 z-50 mt-2 w-64 rounded-2xl border border-gray-200 bg-white p-3 shadow-lg dark:border-gray-700 dark:bg-gray-800"
                    >
                        ...
                        >
                            {{ t("common.analysesDone") }}: {{ analysesCount }}
                        </p>

                        <div class="mt-3 space-y-1">
                            <button
                                class="w-full rounded-xl px-3 py-2 text-left text-sm font-medium text-gray-700 transition hover:bg-gray-100 dark:text-gray-100 dark:hover:bg-gray-700"
                                @click="handleClearHistory"
                            >
                                {{ t("common.clearHistory") }}
                            </button>
                            <button
                                class="w-full rounded-xl px-3 py-2 text-left text-sm font-medium text-red-600 transition hover:bg-red-50 dark:text-red-300 dark:hover:bg-red-900/30"
